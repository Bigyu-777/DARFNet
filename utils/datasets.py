...
def load_image_rgb_ir(self, index, retries=3, delay=0.5):
    # loads 1 image from dataset, returns img, original hw, resized hw
    ...
    if (img_rgb is None) and (img_ir is None):  # not cached
        ...
        for attempt in range(retries):
            try:
                ...
                if img_rgb is None:
                    raise FileNotFoundError(f"Failed to load RGB image from {path_rgb}")
                if img_ir is None:
                    raise FileNotFoundError(f"Failed to load IR image from {path_ir}")

                break  # exit loop if images are loaded successfully
            except Exception as e:
                if attempt < retries - 1:
                    print(f"Warning: {str(e)} - Retrying {attempt + 1}/{retries}...")
                    time.sleep(delay)  # delay before retry
                else:
                    print(f"Error loading images after {retries} attempts: {str(e)}")
                    img_rgb = np.zeros((self.img_size, self.img_size, 3), dtype=np.uint8)
                    img_ir = np.zeros((self.img_size, self.img_size, 3), dtype=np.uint8)
