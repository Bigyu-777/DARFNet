...
class ReciprocalExcite(nn.Module):  # 原 GatedExcitation
    def __init__(self, channel, reduction=16):
        super(ReciprocalExcite, self).__init__()
...
class DivergenceAwareFusion(nn.Module):  # 原 Fusion
    def __init__(self, in_channels, reduction=16):
        super(DivergenceAwareFusion, self).__init__()

        self.rgb_gate = ReciprocalExcite(in_channels, reduction)
        self.ir_gate  = ReciprocalExcite(in_channels, reduction)
        ...
        self.rgb_res_weight = nn.Parameter(torch.ones(1))
        self.ir_res_weight  = nn.Parameter(torch.ones(1))
    ...
    def forward(self, x):
        ...
        rgb_att = self.ir_gate(ir_feat) * rgb_feat
        ir_att  = self.rgb_gate(rgb_feat) * ir_feat

        diff = torch.abs(rgb_feat - ir_feat)
        diff_weight = self.residual_attention(diff)

        rgb_att = rgb_att * diff_weight
        ir_att  = ir_att * diff_weight

        rgb_out = self.fusion_enhancer(rgb_att) + self.rgb_res_weight * rgb_feat
        ir_out  = self.fusion_enhancer(ir_att) + self.ir_res_weight * ir_feat

        return ir_out, rgb_out
