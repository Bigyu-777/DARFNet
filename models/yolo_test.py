...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
                 C3, C3TR,  ODConv_3rd, ConvNextBlock]:
            ...
            args = [c2, args[1]]
        elif m is DivergenceAwareFusion:
            c2 = ch[f[0]]
            args = [c2]
        elif m is Detect:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='models/transformer/yolov5s_fusion_transformerx3_vedai.yaml', help='model.yaml')
    ...
    input_rgb = torch.Tensor(8, 3, 1024, 1024).to(device)
    input_ir = torch.Tensor(8, 3, 1024, 1024).to(device)
